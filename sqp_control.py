...
#     controller = SQPController(env, num_samples=200, horizon=15,
...
def get_cartpole_sqp_hyperparams():
    hyperparams = {
        'Q': np.diag([1.0, 5.0, 5.0, 0.1, 0.1, 0.1]),
        'R': np.array([[0.1]]),
        'horizon': 15,
        'max_iter': 5,
    }
...
class SQPController:
    ...
    def cost(self, U_flat, state):
        ...
        cost_state = np.sum(state_diff @ self.Q * state_diff)
        cost_action = np.sum(U @ self.R * U)
        return cost_state + cost_action

    def command(self, state):
        U_flat = self.U.flatten()

        constraints = []  # No explicit constraints in this simple example
        bounds = [(-10, 10)] * len(U_flat)  # action bounds
